...
Brain Surgeon \cite{hassibiSecondOrderDerivatives1992}
    ...
    space e.g. prune,\hlfix{maybe eliminating the smallest dimensions is not the best strategy.}{This does not make much sense since we are still using magnitude pruning but in a noisy model. So we are actually doing the same but on a different set of weights we just need motivate this differently}
