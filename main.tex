...
\subsection{Outline}
\begin{itemize}
    \item There has been many research about neural network pruning:
...
\item         some of the use second order
    ...
    \item    some of them before training \todo{cite snip grasp and synflow} but they
        can reach the same performance as post training pruning
...
\begin{itemize}
    \item One shot stochastic pruning outperforms one shot deterministic
        pruning
    \item Two step stochastic pruning achieves minimal (state-of-the-art?
        competitive with? ) accuracy degradation without fine-tuning.
\end{itemize}

\subsection{Real Results}%
\label{sub:real_results}
\input{results.tex}
